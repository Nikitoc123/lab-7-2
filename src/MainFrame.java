...
// Убеждаемся, что поля не пустые
            ...
            boolean XIX = false;
            int XAX = 0;
            for (int XEX = 0; XEX < destinationAddress.length(); XEX++)
            {
                if(destinationAddress.charAt(XEX) == '.')
                    XAX++;

                if(destinationAddress.lastIndexOf(".") == destinationAddress.length()-1
                        || destinationAddress.indexOf(".") == 0)
                    XIX = true;

                if (Character.isDigit(destinationAddress.charAt(XEX)) == false && destinationAddress.charAt(XEX) != '.')
                {
                    XIX = true;
                    break;
                }
            ...
            {
                JOptionPane.showMessageDialog(this,
                        "Неадвокатный адрес получателя", "Ошибка",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
